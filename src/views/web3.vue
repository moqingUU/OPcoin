<template>
    <div class="py-24 sm:py-32 lg:py-40" :data-theme="appColor? 'corporate':'halloween'">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="sm:text-center">
                <h2 class="text-lg font-semibold leading-8 text-indigo-600">Transactions</h2>
                <p class="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">A better way to send money
                </p>
            ...
            <div class="mt-20 max-w-lg sm:mx-auto md:max-w-none">
                <div class="grid grid-cols-1 gap-y-16 md:grid-cols-2 md:gap-x-12 md:gap-y-16">
                    ...
                    <div v-for="feature in features" :key="feature.name"
                        ...
                        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-indigo-500 sm:shrink-0">
                            <component :is="feature.icon" class="h-8 w-8" aria-hidden="true" />
                        </div>
                        <div class="sm:min-w-0 sm:flex-1">
                            <p class="text-lg font-semibold leading-8">{{ feature.name }}</p>
                            <p class="mt-2 text-base leading-7">{{ feature.description }}</p>
                        </div>
                    </div>
                </div>
